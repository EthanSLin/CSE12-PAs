 ...
 * Sources used: None
 * 
 * This file contains the framework of an implementation of a ticket object
 */
...
public class Ticket implements Comparable<Ticket>{
    ...
    /**
     * Compares the current ticket and another ticket to see which has priority
     * @param other the Ticket object being compared against
     * @return an order value
     */
    @Override
    public int compareTo(Ticket other){
        if (orderMap.get(ticketType)
        .compareTo(orderMap.get(other.getTicketType())) > 0) {
            return 1;
        }
        else if (orderMap.get(ticketType)
        .compareTo(orderMap.get(other.getTicketType())) < 0) {
            return -1;
        }
        else if (createdAt.compareTo(other.getCreatedAt()) > 0) {
            return 1;
        }
        else if (createdAt.compareTo(other.getCreatedAt()) < 0){
            return -1;
